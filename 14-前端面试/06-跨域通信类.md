...
通源策略：限制从一个源加载的文档或脚本如何与来自另一个源的资源进行交互，这是一个用于隔离潜在恶意文件的关键的安全机制（来自）
具体解释：（1）源 包括三个部分：协议，域名，端口（http协议的默认端口是80）。如果有任何一个部分不同，则源不同，那就是跨域了。
（2）限制：这个源的文档没有权利去操作另一个源的文档。这个限制体现现在（要记住）
...
-Ajax：不支持跨域。
-WebSocket：不受同源策略的限制，支持跨域。
-CORS：不受同源策略的限制，支持跨域。一种新的通信协议标准。同时支持同源和跨域Ajax
...
嘿：发送 Ajax 服务器请求的五个步骤（XMLHttpRequest的工作原理）
...
嘿：跨域通信的几种方式；1，jsonp 2，websocket 3，cors 4，hash 5，postMessage
...
在 CORS 和 postMessage以前，我们一直都是通过JSONP来做跨域通信的。

JSonp原理，通过<script>标签的异步加载来实现的。比如说，实际开发中，我们发现，head标签里，可以通过<script>标签的src，里面放url，加载很多在线的插件，这就是用到了JSONP。
